...
from services.otp_service import OTPGenerator
...
def _get_media_type(content_file):
    if filetype.is_image(content_file):
        valid_media_type = settings.CONTENT_IMAGE_FILE
    elif filetype.is_audio(content_file):
        valid_media_type = settings.CONTENT_AUDIO_FILE
    else:
        valid_media_type = get_content_file_type(get_file_extension(content_file.name))
    return valid_media_type
...
def get_content_file_type(extension):
    file_type = None
    if extension in settings.CONTENT_FILE_IMAGE_TYPES:
        file_type = settings.CONTENT_IMAGE_FILE
    elif extension in settings.CONTENT_FILE_AUDIO_TYPES:
        file_type = settings.CONTENT_AUDIO_FILE
    return file_type
...
def id_file_name(instance, filename):
    return "/".join(
        [
            "files",
            "ID",
...
def product_file_name(instance, filename):
    return "/".join(
        [
            "files",
            "products",
            "{}_{}_{}".format(instance.product_name, get_uuid(), filename).lower(),
        ]
    )
...
def gallery_file_name(instance, filename):
    return "/".join(
        [
            "files",
            "gallery",
            "{}_{}_{}".format(instance.product.product_name, get_uuid(), filename).lower(),
        ]
    )

def brands_file_name(instance, filename):
    return "/".join(
        [
            "files",
            "brands",
            "{}_{}_{}".format(instance.name, get_uuid(), filename).lower(),
        ]
    )

def categories_file_name(instance, filename):
    return "/".join(
        [
            "files",
            "categories",
...
def get_displays(data):
    res = []
    for v in data:
        field = {'name': v[0], 'slug': v[1]}
        res.append(field)
    return res
