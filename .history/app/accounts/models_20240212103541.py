...
class Subscription(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["user", "followers"], name="user_followers_unique_constraint"
            )
        ]
    ...
    def __str__(self):
        return "Subscription {}-{}".format(str(self.user), str(self.followers))

class OneTimePassword(BaseModel):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="user_otp")
    otp = models.CharField(max_length=10, blank=True, db_index=True)
